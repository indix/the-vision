...
var AutoSuggestList =  React.createClass({
  ...
  _getAutoSuggestItem(i, classes, id, text, type){
    return (
      <li className={classnames(classes, "autosuggest-item", {highlighted: this.state.highlighted === i})} data-id={id} data-type={type} onMouseEnter={this._onItemHoverIn} onClick={this._onSelect} key={i}>{text}</li>
    );
  },
  ...
  render: function(){
    ...
    if(!this.props.loading && !this.props.empty && (!data || _.isEmpty(data))){
      return (
        <ul className="ix-autosuggest">
          <li className="autosuggest-item">No Suggestions Found</li>
        </ul>
      );
    }

    if(this.props.empty && (!data || !data.length))
      return (<ul className="ix-autosuggest"><li className="autosuggest-item">Type something...</li></ul>);

    if(this.props.loading)
      return (<ul className="ix-autosuggest"><li className="autosuggest-item">Loading...</li></ul>);
    ...
    if(type.length === 1)
      ...
      for(var j = 0; j < type.length; j++){
        var t = type[j];
        var dataItem = data[t] || [];
        items.push(<li className={classnames("autosuggest-header", pbcs[t].className)} key={t+j}>{pbcs[t].plural}</li>);
    ...
    return (
      <ul className="ix-autosuggest" onMouseLeave={this._onHoverOut}>
        {items}
      </ul>
    );
  }
